...
std::vector<int> GraphAlgorithms::BreadthFirstSearch(Graph &graph,
  ...
  while (!q.empty()) {
    int src = q.front();
    if (std::find(traversed_vertices.begin(), traversed_vertices.end(), src) ==
        traversed_vertices.end()) {
      traversed_vertices.push_back(src);
      for (int dest : graph.Destinations(src)) {
        q.push(dest);
      }
...
double GraphAlgorithms::GetShortestPathBetweenVertices(Graph &graph,
  ...
  std::vector<double> distances(
      num_vertices,
      std::numeric_limits<double>::infinity());  // длины кратчайших путей
  distances[vertex1] = 0;
...
GraphAlgorithms::GetShortestPathsBetweenAllVertices(Graph &graph) {
  int num_vertices = graph.GetSize();
  std::vector<std::vector<double>> shortest_paths(
      num_vertices, std::vector<double>(
                        num_vertices, std::numeric_limits<double>::infinity()));
...
std::vector<std::vector<double>> GraphAlgorithms::GetLeastSpanningTree(
    Graph &graph) {
  std::vector<std::vector<double>> MST(graph.GetSize(),
                                       std::vector<double>(graph.GetSize(), 0));
  std::vector<int> traversed_vertices;
  int src = 0;
  int dest = 0;
  ...
  for (int i = 0; i < graph.GetSize(); i++) {
    for (int cur_src : traversed_vertices) {
      for (int cur_dest : graph.Destinations(cur_src)) {
        if (std::count(traversed_vertices.begin(), traversed_vertices.end(),
                       cur_dest))
          continue;
...
TsmResult GraphAlgorithms::SolveTravelingSalesmanProblem(Graph &graph) {
  ...
  std::vector<std::vector<double>> pheromones(
      graph.GetSize(), std::vector<double>(graph.GetSize(), 0.2));
  std::vector<std::vector<double>> add_pher(
      graph.GetSize(), std::vector<double>(graph.GetSize(), 0));
  srand(time(0));
  int src = 0;
  ...
  for (auto i = 1; i < 2400; i++) {
    ...
    if (i % ants == 0) updatePheromones(pheromones, add_pher);
  }
  if ((int)result.vertices.size() != graph.GetSize() + 1 ||
      result.distance == std::numeric_limits<double>::infinity())
    throw std::string("it is impossible to solve");
  return result;
}

TsmResult GraphAlgorithms::createRoute(
    Graph &graph, std::vector<std::vector<double>> &pheromones, int src) {
  TsmResult res{};
  std::vector<bool> visited(graph.GetSize(), false);
  res.vertices.push_back(src);
  for (auto i = 0; i < graph.GetSize() - 1; i++) {
    visited[src] = true;
    auto probabilityDest =
        transitionProbabilities(graph, pheromones, visited, src);
...
std::vector<double> GraphAlgorithms::transitionProbabilities(
    Graph &graph, std::vector<std::vector<double>> &pheromones,
...
int GraphAlgorithms::chooseNextDestination(
    std::vector<double> &probabilityToVertex) {
  double choose = (double)rand() / RAND_MAX;
  for (size_t dest = 0; dest < probabilityToVertex.size(); dest++) {
    if (probabilityToVertex[dest] > choose) return dest;
  }
  return -1;
}

void GraphAlgorithms::addPheromone(TsmResult &ant_route,
                                   std::vector<std::vector<double>> &add_pher) {
  int q = 12;
