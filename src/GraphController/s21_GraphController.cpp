...
void GraphController::ShortestPathBetweenVertices(int start_vertex, int end_vertex) {
    double shortestPath = algorithms.GetShortestPathBetweenVertices(graph, start_vertex, end_vertex);
...
void GraphController::ShortestPathsBetweenAllVertices() {
    std::vector<std::vector<double>> allShortestPaths = algorithms.GetShortestPathsBetweenAllVertices(graph);
    std::cout << "Shortest Paths Between All Vertices:" << std::endl;
    for (const auto& row : allShortestPaths) {
        for (double path : row) {
            std::cout << (path == std::numeric_limits<double>::infinity() ? "INF" : std::to_string(path)) << " ";
        }
        std::cout << std::endl;
    }
}
